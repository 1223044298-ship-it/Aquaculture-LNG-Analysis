...
def create_tinted_icon_array(icon_arr, color_segments, outline_thickness=3):
    ...
    for rgb_color, frac in color_segments:
        if frac <= 0:
            continue
        x_end = x_start + int(round(frac * tinted_arr.shape[1]))
        x_end = min(x_end, tinted_arr.shape[1])
        tinted_arr[:, x_start:x_end, :3] = rgb_color
        x_start = x_end

    if x_start < tinted_arr.shape[1] and color_segments:
...
def plot_icon_panel(df, icon_path, colors_hex, output_path,
    ...
    required_cols = [
        "Country",
        "Open_Flow_population",
        "Dense_Array_population",
        "Semi_Enclosed_population"
    ]
    ...
    df["seg_Open_Flow"] = df["Open_Flow_population"].clip(lower=0)

    df["seg_Dense_Array_add"] = (
        df["Dense_Array_population"] - df["Open_Flow_population"]
    ).clip(lower=0)

    df["seg_Semi_Enclosed_add"] = (
        df["Semi_Enclosed_population"] - df["Dense_Array_population"]
    ).clip(lower=0)

    df["total"] = df[
        ["seg_Open_Flow", "seg_Dense_Array_add", "seg_Semi_Enclosed_add"]
    ].sum(axis=1)
    ...
    if len(rest_df) > 0:
        others = pd.DataFrame({
            "Country": ["Others"],
            "seg_Open_Flow": [rest_df["seg_Open_Flow"].sum()],
            "seg_Dense_Array_add": [rest_df["seg_Dense_Array_add"].sum()],
            "seg_Semi_Enclosed_add": [rest_df["seg_Semi_Enclosed_add"].sum()],
    ...
    for _, row in plot_df.iterrows():
        segments = [
            row["seg_Open_Flow"],
            row["seg_Dense_Array_add"],
            row["seg_Semi_Enclosed_add"]
        ]
